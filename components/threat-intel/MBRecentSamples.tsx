        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            {(Object.entries(fileTypeStats) as [string, number][]).slice(0, 3).map(([type, count]) => (
              <div key={type} className="px-2 py-1 border border-border bg-card/50 rounded text-xs text-muted-foreground">
                {type}: {count}
              </div>
            ))}
          </div>
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg flex items-center gap-2 ${
              refreshing
                ? 'bg-muted text-muted-foreground cursor-not-allowed'
                : 'border border-border bg-card/50 hover:border-border hover:text-foreground text-muted-foreground'
            }`}
          >
            <RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''}`} />
            Refresh
        ...
        {filteredSamples.slice(0, 12).map((sample, index) => {
          ...
          return (
            <div
              ...
              className="border border-border bg-card rounded-lg overflow-hidden hover:border-primary/30 transition-colors"
            >
              {/* Sample Header */}
              <div className="p-4">
      ...
      {filteredSamples.length > 12 && (
        <div className="text-center pt-4">
          <button
            ...
            className="px-4 py-2 border border-border bg-card/50 rounded-lg hover:border-border hover:text-foreground text-muted-foreground transition-colors flex items-center gap-2 mx-auto"
          >
