...
class APIService {
  ...
  async uploadFile(
    ...
    console.log(
      `📤 Uploading file for ${analysisType} analysis to ${endpoint}`
    );
  ...
  async getAllReports() {
    try {
      ...
      if (!response.ok) {
        console.warn(
          "Could not fetch reports from backend, using fallback data"
        );
      ...
      console.warn(
        "Network error fetching reports, using fallback data:",
        error
      );
      return this.getMockReports();
    }
  ...
  async getAnalysisComponents(analysisId: string) {
    try {
      const response = await fetch(
        `${this.baseUrl}/analysis/${analysisId}/components`
      );
  ...
  async getCapeReport(analysisId: string) {
    try {
      const response = await fetch(
        `${this.baseUrl}/analysis/${analysisId}/cape`
      );
    ...
    try {
      const url =
        sectionName && sectionName !== "all"
          ? `${
              this.baseUrl
            }/analysis/${analysisId}/parsed?section=${encodeURIComponent(
              sectionName
            )}`
          : `${this.baseUrl}/analysis/${analysisId}/parsed`;
  ...
  async getAiAnalysis(analysisId: string, sectionName: string = "summary") {
    try {
      const url =
        sectionName && sectionName !== "summary"
          ? `${
              this.baseUrl
            }/analysis/${analysisId}/ai?section=${encodeURIComponent(
              sectionName
            )}`
          : `${this.baseUrl}/analysis/${analysisId}/ai`;
